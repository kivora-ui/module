...
it("exports supported native families and keeps a gallery example", () => {
  const webExports = readFileSync(resolve("../nextjs/src/index.ts"), "utf8");
  const web = readdirSync(resolve("../nextjs/src/components")).filter(name => name.endsWith(".tsx") && !name.includes(".test.") && webExports.includes(`./components/${name.slice(0, -4)}`));
  ...
  const desktopOnly = ["hover-card.tsx", "upload-dashboard.tsx", "resizable.tsx", "command.tsx", "context-menu.tsx", "navigation-menu.tsx"];
