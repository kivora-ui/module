...
import { ActivityIndicator, AppState, Image, Pressable, ScrollView, StyleSheet, Text, View, type ViewProps } from 'react-native';
...
import { Play, Pause, RotateCcw, RotateCw, Volume2, VolumeX, Settings2, Maximize, ListVideo, X, EllipsisVertical } from 'lucide-react-native';
import Svg, { Defs, LinearGradient, Stop, Rect } from 'react-native-svg';
import { Slider } from '../slider';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '../tabs';
import { BottomSheet } from '../bottom-sheet';
import { PlayerController, playerTime } from './controller';
import { CastConnection, type PlayerCast } from './cast-connection';
import { AudioPresentation } from './audio-presentation';
import { useNativeAudio } from './audio-native';
import { castUnsupportedReason, type CastBridge } from './cast';
...
export function Player({ source, controller: provided, autoPlay = false, cast, airPlay = false, settingsLayout = 'tabs', orientation = 'auto', muted = false, locale = 'es', controlsVariant = 'compact', program, queue = [], activeQueueId, onQueueSelect, onClose, presentation = 'inline', style, ...props }: PlayerProps) {
  const [local] = React.useState(() => new PlayerController());
  const controller = provided ?? local;
  const state = usePlayer(controller);
  useNativeAudio(controller, state);
  const video = React.useRef<VideoRef>(null);
  const castBridge = React.useRef<CastBridge | undefined>(undefined);
  const [airPlaying, setAirPlaying] = React.useState(false);
  const remote = !!state.casting;
  const unsupportedCast = cast ? castUnsupportedReason(source) : undefined;
  const pauseLocal = React.useCallback(() => { video.current?.pause(); video.current?.dismissFullscreenPlayer(); setFullscreen(false); }, []);
  const [displayedRevision, setDisplayedRevision] = React.useState<number>();
  ...
  const queueVisible = !audio && !intro && (mobile ? mobileQueueOpen : queueOpen) && queue.length > 0 && !state.ad;
  const wake = () => { setIdle(false); setActivity(value => value + 1); };
  React.useEffect(() => controller.connect(time => { if (controller.getSnapshot().casting && castBridge.current) castBridge.current.seek(time); else video.current?.seek(time); }, () => { castBridge.current?.stop(); video.current?.pause(); video.current?.dismissFullscreenPlayer(); }), [controller]);
  React.useEffect(() => controller.on(event => {
    if (event.type === 'close') { setFullscreen(false); setSettings(false); onClose?.(); }
  }), [controller, onClose]);
  React.useEffect(() => { controller.loadSource(source, autoPlay); }, [controller, source, autoPlay]);
  React.useEffect(() => {
    if (!remote && !audio && orientation === 'landscape' && !state.paused && state.mediaUri) setFullscreen(true);
  }, [audio, remote, orientation, state.paused, state.mediaUri]);
  ...
  React.useEffect(() => {
    const subscription = AppState.addEventListener('change', next => { if (next === 'active') controller.checkSleepTimer(); });
    return () => subscription.remove();
  }, [controller]);
  React.useEffect(() => {
    if (audio) return;
    const subscription = AppState.addEventListener('change', next => { if (next !== 'active' && !controller.getSnapshot().casting && !airPlaying) controller.pause(); });
    return () => subscription.remove();
  }, [audio, controller, airPlaying]);
  React.useEffect(() => {
  ...
  React.useEffect(() => setAirPlaying(false), [revision]);
  const current = () => !controller.getSnapshot().casting && controller.getSnapshot().revision === revision;
  const nativeSource = React.useMemo(() => state.mediaUri ? {
    ...(!intro && (!state.ad || state.ad.ima) ? source.nativeSource : {}),
    uri: state.mediaUri,
    ...
    ...(audio ? { metadata: { title: state.ad?.title || source.title, artist: source.subtitle, imageUri: source.poster } } : {}),
    ...(!intro && !state.ad && source.ads?.tagUrl ? { ad: { type: 'csai' as const, adTagUrl: source.ads.tagUrl } } : {}),
  ...
  const castButton = CastButton ? <CastButton accessibilityLabel="Google Cast" tintColor="white" style={{ width: 40, height: 40 }} /> : null;
  const transport = <>
  ...
  const controls = <View style={styles.controls}>
    ...
    <View style={styles.timeline}>
      ...
      <View style={styles.seek}><Slider accessibilityLabel={t.seek} value={[state.currentTime]} min={0} max={state.duration || 1} step={1} disabled={intro || !!state.ad || state.duration <= 0} onValueChange={values => { wake(); controller.seek(values[0] ?? 0); }} trackClassName="bg-white/30" rangeClassName="bg-rose-500" thumbClassName="bg-rose-500" /></View>
    ...
    <View style={styles.toolbar}>
      ...
      <IconButton label={t.settings} disabled={remote} onPress={() => { setSettings(true); wake(); }}><Settings2 color="white" size={21} /></IconButton>
      {!audio && <IconButton label={t.fullscreen} disabled={remote || !!state.ad} onPress={() => setFullscreen(true)}><Maximize color="white" size={21} /></IconButton>}
    </View>
  </View>;
  ...
  return <View {...props} testID={props.testID ?? (audio ? 'audio-player' : 'video-player')} style={[!audio && styles.root, style]} onLayout={event => { setWidth(event.nativeEvent.layout.width); props.onLayout?.(event); }}>
    ...
    <View style={audio ? styles.audioStage : styles.stage}>
      <Video key={revision} ref={video} source={remote ? undefined : nativeSource} style={audio ? styles.audioMedia : StyleSheet.absoluteFill} resizeMode={ResizeMode.CONTAIN} controls={fullscreen || !!state.ad?.ima} fullscreen={fullscreen} fullscreenOrientation={orientation === 'landscape' ? 'landscape' : 'all'} fullscreenAutorotate={orientation === 'auto'} paused={remote || state.paused} muted={state.muted} volume={state.volume} rate={state.ad ? 1 : state.rate} allowsExternalPlayback={airPlay} playInBackground={audio || (airPlay && airPlaying)} playWhenInactive={audio || (airPlay && airPlaying)} showNotificationControls={audio && !remote} ignoreSilentSwitch="ignore" progressUpdateInterval={250}
        ...
        onExternalPlaybackChange={data => setAirPlaying(data.isExternalPlaybackActive)}
        onLoad={data => { if (current()) controller.loaded(data); }}
        onReadyForDisplay={() => { if (current()) setDisplayedRevision(revision); }}
        ...
        onPlaybackStateChanged={data => { if (current() && !data.isSeeking) { if (data.isPlaying) controller.play(); if (audio || fullscreen) controller.playback(data.isPlaying); } }}
        ...
        onAudioBecomingNoisy={() => { if (current()) controller.pause(); }}
        onReceiveAdEvent={data => { if (current() && data.event) controller.imaEvent(data.event); }}
      />
      {!audio && source.poster && displayedRevision !== revision && state.phase !== 'error' && <Image testID="player-loading-image" accessibilityLabel={t.loading} pointerEvents="none" source={{ uri: source.poster }} resizeMode="contain" style={[StyleSheet.absoluteFill, { backgroundColor: 'black' }]} />}
      {!audio && <>
      ...
      {mobile && !idle && <View pointerEvents="box-none" style={StyleSheet.absoluteFill}>
        ...
        <View style={styles.mobileActions}>
          {castButton}
          <IconButton round label={t.fullscreen} disabled={remote || !!state.ad} onPress={() => setFullscreen(true)}><Maximize color="white" size={18} /></IconButton>
          ...
          <IconButton round label={t.settings} disabled={remote} onPress={() => { setSettings(true); wake(); }}><EllipsisVertical color="white" size={19} /></IconButton>
        </View>
        {!queueVisible && !state.ad && !state.buffering && ['intro', 'content', 'ended'].includes(state.phase) && <View pointerEvents="box-none" style={[styles.center, styles.mobileTransport]}>
          <IconButton round disabled={intro} label={t.back} onPress={() => { controller.seek(state.currentTime - 10); wake(); }}><RotateCcw color="white" size={23} /><Text style={styles.skipNumber}>10</Text></IconButton>
          ...
          <IconButton round disabled={intro} label={t.forward} onPress={() => { controller.seek(state.currentTime + 10); wake(); }}><RotateCw color="white" size={23} /><Text style={styles.skipNumber}>10</Text></IconButton>
        </View>}
        <View style={styles.mobileBottom}>
          {state.ad ? <View style={styles.adRow}><IconButton label={state.paused ? t.play : t.pause} onPress={toggle}>{state.paused ? <Play color="white" size={20} /> : <Pause color="white" size={20} />}</IconButton><Text numberOfLines={1} style={styles.adText}>{adLabel}</Text>{state.ad.canSkip && <Pressable accessibilityRole="button" onPress={controller.skipAd} style={styles.pill}><Text style={styles.white}>{t.skip}</Text></Pressable>}</View> : <>
            <View style={styles.mobileCaption}><View style={styles.spacer}><Text numberOfLines={1} style={styles.mobileTitle}>{intro ? source.intro?.title ?? source.title : program?.title ?? source.title}</Text>{!intro && program?.subtitle && <Text numberOfLines={1} style={styles.mobileSubtitle}>{program.subtitle}</Text>}</View><Text style={styles.time} testID="video-time">{playerTime(state.currentTime)} / {playerTime(state.duration)}</Text></View>
            <Slider accessibilityLabel={t.seek} value={[state.currentTime]} min={0} max={state.duration || 1} step={1} disabled={intro || state.duration <= 0} onValueChange={values => { wake(); controller.seek(values[0] ?? 0); }} style={styles.mobileSeek} trackClassName="h-0.5 bg-white/30" rangeClassName="bg-white" thumbClassName="h-2.5 w-2.5 bg-white border-white" />
          </>}
        </View>
      </View>}
      {!audio && (state.phase === 'loading' || state.buffering) && <View pointerEvents="none" style={styles.center}><ActivityIndicator accessibilityLabel={t.loading} color="white" /></View>}
      {!audio && state.phase === 'error' && <View style={styles.error}><Text style={styles.white}>{t.error}</Text><Pressable accessibilityRole="button" onPress={controller.retry} style={styles.pill}><Text style={styles.white}>{t.retry}</Text></Pressable></View>}
    </View>
    {!audio && (remote || airPlaying || unsupportedCast || state.castError) && <Text accessibilityLiveRegion="polite" style={[styles.white, { padding: 8 }]}>{state.castError
      ? (locale === 'es' ? 'Google Cast no ha podido reproducir el contenido. Desconecta Cast para volver al dispositivo.' : 'Google Cast could not play this media. Disconnect Cast to return to this device.')
      : unsupportedCast ? (locale === 'es' ? 'Cast no disponible: requiere URL remota accesible y no admite archivos locales, DRM, cabeceras, subtítulos externos ni IMA.' : 'Cast unavailable: requires reachable remote URLs; local files, DRM, headers, external subtitles and IMA are unsupported.')
      : remote ? (locale === 'es' ? 'Reproduciendo en Google Cast' : 'Playing on Google Cast') : 'AirPlay'}</Text>}
    {!audio && !mobile && <View pointerEvents={idle && !audio && controlsVariant !== 'standard' ? 'none' : 'auto'} style={{ opacity: idle && !audio && controlsVariant !== 'standard' ? 0 : 1 }}>{controls}</View>}
    ...
    {audio && <AudioPresentation source={source} state={state} controller={controller} cast={cast} locale={locale} onClose={onClose} footer={presentation === 'footer'} />}
  </View>;
}
