...
Renderiza códigos QR SVG desde el motor compartido de `@kivora/codes`.
...
```tsx
import { QRCode } from "@kivora/nextjs";
```

### @kivora/native

```tsx
import { QRCode } from "@kivora/native";
```
...
- **Componentes y helpers visuales:** `QRCode`
- **Tipos de props:** `QRCodeProps`
- **Tipos relacionados:** —
...
- **Definido en:** `packages/nextjs/src/components/qr-code.tsx`
- **Composición base:** `Omit<BarcodeProps, 'format' | 'width' | 'height'>`

**Props propias**

| Prop | Tipo | Opcional | Descripción |
| --- | --- | --- | --- |
| `size` | `number | undefined` | sí | — |

<details>
<summary>Props heredadas o compuestas de `QRCodeProps` (284)</summary>
...
| `background` | `string | undefined` | sí | — | `packages/codes/src/index.ts` |
...
| `displayValue` | `boolean | undefined` | sí | — | `packages/nextjs/src/components/barcode.tsx` |
...
| `errorCorrectionLevel` | `QRErrorCorrectionLevel | undefined` | sí | QR error correction. Defaults to M. Ignored for other formats. | `packages/codes/src/index.ts` |
| `exportparts` | `string | undefined` | sí | — | `node_modules/.pnpm/@types+react@19.2.18/node_modules/@types/react/index.d.ts` |
| `fallback` | `string | number | bigint | boolean | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<AwaitedReactNode> | null | undefined` | sí | — | `packages/nextjs/src/components/barcode.tsx` |
| `foreground` | `string | undefined` | sí | Opaque #RRGGBB colors. Defaults to black on white, independent of theme. | `packages/codes/src/index.ts` |
...
| `margin` | `number | undefined` | sí | Quiet zone in unscaled SVG units. Minimum 4 for QR, 10 for linear codes. | `packages/codes/src/index.ts` |
...
| `onError` | `((error: Error) => void) | undefined` | sí | — | `packages/nextjs/src/components/barcode.tsx` |
...
| `value` | `string` | no | Exact content to encode. Leading zeroes and whitespace are preserved. | `packages/codes/src/index.ts` |
...
- **Definido en:** `packages/native/src/components/qr-code.tsx`
- **Composición base:** `Omit<BarcodeProps, 'format' | 'width' | 'height'>`
...
<details>
<summary>Props heredadas o compuestas de `QRCodeProps` (123)</summary>
...
| `displayValue` | `boolean | undefined` | sí | — | `packages/native/src/components/barcode.tsx` |
| `errorCorrectionLevel` | `QRErrorCorrectionLevel | undefined` | sí | QR error correction. Defaults to M. Ignored for other formats. | `packages/codes/src/index.ts` |
...
| `fallback` | `string | number | bigint | boolean | React.ReactElement<unknown, string | React.JSXElementConstructor<any>> | Iterable<React.ReactNode> | React.ReactPortal | Promise<AwaitedReactNode> | null | undefined` | sí | — | `packages/native/src/components/barcode.tsx` |
...
| `onError` | `((error: Error) => void) | undefined` | sí | — | `packages/native/src/components/barcode.tsx` |
